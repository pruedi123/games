...
def say_word_repeat(word: str, times: int = 3, slow: bool = True):
    """Speak the word multiple times in a row. Uses slow pace by default for kids."""
...
def render_listen_and_spell():
    ...
    if st.session_state.get("_repeat_word"):
        say_word_repeat(target, times=int(st.session_state.get("_repeat_times", 3)), slow=bool(st.session_state.get("_repeat_slow", True)))
    ...
    if submitted:
        ...
        if guess.strip().lower() == target:
            ...
            # Immediately clear the input and auto-repeat the word 3x slowly
            st.session_state["_clear_listen"] = True

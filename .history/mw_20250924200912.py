...
def fetch_mw_audio_for_word(word: str, api_key: str, dict_name: str = "learners") -> tuple[bool, str | Path]:
    ...
    try:
        ...
        return False, f"HTTP {e.code} for {dict_name}. Check that your key matches this dictionary."
    ...
    if not audio_code:
        # Try to surface a helpful hint if we got suggestions instead of entries
        try:
            if data and isinstance(data[0], str):
                return False, "No audio (suggestions returned). Try different dictionary or word form."
        except Exception:
            pass
    ...
    try:
        ...
        return False, f"Audio CDN HTTP {e.code}"
    except URLError as e:
        return False, f"Audio CDN network error: {e.reason}"
    except Exception as e:
...
if col_mw1.button("Fetch MW audio for this list", use_container_width=True, disabled=not bool(api_key)):
    ...
    for w in words:
        ok1, msg = fetch_mw_audio_for_word(w, api_key, dict_name)
        if ok1:
            ok += 1; new_log.append(f"✔ {w} → saved")
        else:
            fail.append(f"{w} ({msg})"); new_log.append(f"✖ {w} → {msg}")
    st.session_state["mw_log"] = new_log
    if ok:
        st.success(f"Downloaded {ok} MW audio file(s) to {AUDIO_MW_DIR}.")
        st.caption(f"Saved in: {AUDIO_MW_DIR}")
        st.session_state.last_spoken_idx = -1
    if fail:
        st.warning("Issues: " + ", ".join(fail[:6]) + (" …" if len(fail) > 6 else ""))
...
if col_mw2.button("Clear audio for this list", use_container_width=True):
    try:
        ...
        st.session_state["mw_log"] = ["(cleared)"]
    except Exception as e:
        st.sidebar.error(f"Clear failed: {e}")

# One‑word tester (current word)
col_mw3, col_mw4 = st.sidebar.columns(2)
if col_mw3.button("Test fetch for current word", use_container_width=True, disabled=not bool(api_key)):
    try:
        cur = st.session_state.words[st.session_state.idx]
        ok1, msg = fetch_mw_audio_for_word(cur, api_key, dict_name)
        if ok1:
            st.success(f"Saved MW audio for '{cur}'."); st.session_state.last_spoken_idx = -1
        else:
            st.warning(f"{cur}: {msg}")
    except Exception as e:
        st.sidebar.error(f"Tester failed: {e}")
if col_mw4.button("Open MW folder path", use_container_width=True):
    st.info(f"MW files: {AUDIO_MW_DIR}")

# Show debug log
if st.session_state.get("mw_log"):
    with st.sidebar.expander("MW fetch log", expanded=False):
        for line in st.session_state["mw_log"]:
            st.code(line)
...
if HAS_GTTS:
    col_wa1, col_wa2 = st.sidebar.columns(2)
    if col_wa1.button("Generate TTS word files for this list"):
        # Save into AUDIO_MW_DIR so local-preferred path picks them up
        outdir = AUDIO_MW_DIR
        outdir.mkdir(parents=True, exist_ok=True)
        made, fails = 0, []
        for w in st.session_state.words:
        ...
        if made:
            ...
            st.caption(f"Local files will be used first. Folder: {AUDIO_MW_DIR}")
            st.session_state.last_spoken_idx = -1  # ensure next click picks up local audio
        if fails:
            st.warning("Failed: " + ", ".join(fails))

    # Helper: move any existing teacher TTS files for the current list into MW folder
    if col_wa2.button("Move existing TTS → MW for this list"):
        moved, missing = 0, []
        tts_dir = get_audio_dir()
        AUDIO_MW_DIR.mkdir(parents=True, exist_ok=True)
        for w in st.session_state.words:
            wl = w.lower()
            found = None
            ...
            if found is not None:
                ...
                missing.append(w)
        st.success(f"Moved {moved} file(s) into {AUDIO_MW_DIR}")
        if missing:
            st.caption("No TTS file found for: " + ", ".join(missing[:8]) + (" …" if len(missing) > 8 else ""))
else:
...
# MW local file location reminder
if p_preview is not None and src == "MW":
    st.caption(f"MW file found in: {AUDIO_MW_DIR}")

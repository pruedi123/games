...
if idx >= N:
    ...
    st.markdown(f"**Final Jupiter Points:** {st.session_state.current_points} / {st.session_state.total_points}")
...
with st.container(border=True):
    ...
    if cc3.button("Next word ▶", use_container_width=True):
        if st.session_state.idx < len(st.session_state.words) - 1:
            st.session_state.idx += 1
            st.session_state.attempted_penalty = False
            st.session_state.last_feedback = ""
            st.session_state._retry_speak = False
            st.session_state.last_spoken_idx = -1  # allow auto-play on next word
            st.session_state.listen_nonce += 1
        else:
            # move to completion state
            st.session_state.idx = len(st.session_state.words)
            st.session_state.last_feedback = "✅ All words complete!"
...
if submitted:
    ...
    if correct:
        ...
        st.session_state.last_feedback = "✅ You were right!"
    else:
        say_feedback("Try again", kinder)
        # Deduct one Jupiter Point for a miss (no recovery in this simplified mode)
        st.session_state.current_points = max(0, st.session_state.current_points - 1)
        st.session_state.last_feedback = "❌ Try again next time."

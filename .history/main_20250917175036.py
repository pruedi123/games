...
# Allow a session override for the audio directory so we can reuse files created by other scripts
def get_audio_dir() -> Path:
    p = st.session_state.get("audio_dir_override")
    if p:
        try:
            return Path(p)
        except Exception:
...
def play_audio_file(path: str):
    ...
    st.components.v1.html(
        f"""
        <audio autoplay>
          <source src="file://{path}" type="{mime}">
        </audio>
        """,
        height=0,
    )
...
# Helper to import/copy audio files that already exist from another folder (e.g., created by your other scripts)
import shutil

def import_audio_from_folder(words: list[str], src_dir: str) -> tuple[int, list[str]]:
    """Copy audio files for given words from src_dir into our audio dir. Returns (copied_count, misses)."""
    ensure_pron_dir()
    src = Path(src_dir)
    dst = get_audio_dir()
    copied = 0
    ...
    for w in words:
        found = None
        for ext in (".mp3", ".wav", ".m4a"):
            candidate = src / f"{w.lower()}{ext}"
            if candidate.exists():
                found = candidate
                break
        if not found:
            # fallback: any file starting with the word
            for p in src.glob(f"{w.lower()}*.*"):
                if p.suffix.lower() in (".mp3", ".wav", ".m4a"):
                    found = p
                    break
        if found:
            try:
                shutil.copy2(found, dst / f"{w.lower()}{found.suffix.lower()}")
                copied += 1
            except Exception:
                misses.append(w)
        else:
...
# Official Collegiate Dictionary API: https://dictionaryapi.com/
...
def mw_subdir(audio_name: str) -> str:
    if audio_name.startswith("bix"):
        return "bix"
    if audio_name.startswith("gg"):
        return "gg"
    if re.match(r"^[0-9]", audio_name):
        return "number"
    return audio_name[0]

def mw_best_audio(entry: dict) -> str | None:
    try:
...
def mw_fetch_audio_url(word: str, api_key: str) -> str | None:
    try:
        resp = requests.get(f"{MW_API_BASE}{word}", params={"key": api_key}, timeout=5)
        ...
        for item in data:
            if isinstance(item, dict):
                audio_name = mw_best_audio(item)
                if audio_name:
                    break
        if not audio_name:
            return None
        sub = mw_subdir(audio_name)
        return f"{MW_AUDIO_BASE}{sub}/{audio_name}.mp3"
    except Exception:
        return None

def play_audio_url(url: str):
    # Autoplay a remote MP3 URL in the page
    st.components.v1.html(
        f"""
        <audio autoplay>
...
def say_word_now(word: str):
    ...
    if lp:
        play_audio_file(lp)
        return
    # 2) Merriam‑Webster (preferred when selected or Auto with key)
    use_mw = st.session_state.get("pron_source") in ("Merriam-Webster", "Auto (Best)") and bool(st.session_state.get("mw_api_key"))
    if use_mw:
        mw_url = mw_fetch_audio_url(word, st.session_state.get("mw_api_key"))
        if mw_url:
            play_audio_url(mw_url)
            return
...
def say_word_repeat(
    ...
    if lp:
        ...
        return
    # 2) Merriam‑Webster loop with gaps
    use_mw = st.session_state.get("pron_source") in ("Merriam-Webster", "Auto (Best)") and bool(st.session_state.get("mw_api_key"))
    if use_mw:
        mw_url = mw_fetch_audio_url(word, st.session_state.get("mw_api_key"))
    else:
        ...
        st.components.v1.html(
            ...
            <script>
              (function() {{
                ...
                var count = 0;
                var gap = {gap_ms};
                var audio = new Audio({json.dumps(mw_url)!r});
                audio.addEventListener('ended', function() {{
                  count += 1;
                  if (count < times) {{
                    setTimeout(function() {{ audio.currentTime = 0; audio.play(); }}, gap);
...
def sidebar_controls():
    ...
    if st.session_state.pron_source in ("Merriam-Webster", "Auto (Best)"):
        if st.sidebar.button("Download MW audio for this list (saves to current audio folder)", use_container_width=True, disabled=not bool(st.session_state.get("mw_api_key"))):
            if not st.session_state.get("mw_api_key"):
                ...
                if dl:
                    st.success(f"Downloaded {dl} audio file(s) to {get_audio_dir()}.")
                if misses:
                    st.info("No MW audio found for: " + ", ".join(misses))

    # Use an existing folder of audio files (e.g., created by your other script)
    st.sidebar.text("\n")
    st.sidebar.caption("Use audio files that already exist on disk (e.g., from another script).")
    audio_dir_inp = st.sidebar.text_input("Existing audio folder (optional)", value=st.session_state.get("audio_dir_override", str(PRON_DIR)))
    col_imp1, col_imp2 = st.sidebar.columns([1,1])
    if col_imp1.button("Set as audio folder", use_container_width=True):
        st.session_state.audio_dir_override = audio_dir_inp
        st.session_state.use_local_audio = True
        st.success(f"Audio folder set to: {audio_dir_inp}")
    if col_imp2.button("Import files for this list", use_container_width=True):
        copied, misses = import_audio_from_folder(words, audio_dir_inp)
        st.session_state.use_local_audio = True
        if copied:
            st.success(f"Copied {copied} file(s) into {get_audio_dir()}")
        if misses:
            st.info("Missing in source: " + ", ".join(misses))

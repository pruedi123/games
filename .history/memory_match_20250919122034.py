...
# Locate audio files next to this script and load as base64 data URIs
SOUND_DIR = Path(__file__).resolve().parent

@st.cache_data(show_spinner=False)
def _load_data_uri(filename: str):
    p = SOUND_DIR / filename
    if not p.exists():
        return None
    b = p.read_bytes()
    return "data:audio/mpeg;base64," + base64.b64encode(b).decode()

if "sound_match_uri" not in st.session_state:
    st.session_state.sound_match_uri = _load_data_uri("match.mp3")
if "sound_fail_uri" not in st.session_state:
    st.session_state.sound_fail_uri = _load_data_uri("fail.mp3")
...
# --- Simple sound player (plays a preloaded data URI)
def play_sound_uri(uri: str):
    if not uri:
        return
...
if sound_enabled:
    ...
    if tc1.button("Test match"):
        play_sound_uri(st.session_state.sound_match_uri)
    if tc2.button("Test fail"):
        play_sound_uri(st.session_state.sound_fail_uri)

if sound_enabled and (st.session_state.sound_match_uri is None or st.session_state.sound_fail_uri is None):
    missing = []
    if st.session_state.sound_match_uri is None:
        missing.append("match.mp3")
    if st.session_state.sound_fail_uri is None:
        missing.append("fail.mp3")
    st.sidebar.warning("Missing audio file(s): " + ", ".join(missing))

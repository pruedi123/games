...
def find_local_audio_for_word(word: str) -> Path | None:
    # First check the teacher‑configured folder (audio_tts by default)
    base = get_audio_dir()
    if base.exists():
        wl = word.lower()
        for ext in AUDIO_EXTS:
        ...
        for ext in AUDIO_EXTS:
            for p in base.glob(f"*{wl}*{ext}"):
                return p
    # Then check the MW downloads directory
    wl = word.lower()
    if AUDIO_MW_DIR.exists():
        for ext in AUDIO_EXTS:
            p = AUDIO_MW_DIR / f"{wl}{ext}"
            if p.exists():
                return p
        for ext in AUDIO_EXTS:
            for p in AUDIO_MW_DIR.glob(f"{wl}*{ext}"):
                return p
    return None

...
def play_ui_sound(name: str, rate: float = 1.0):
    ...
    if path is not None:
        ...
        try:
            ...
            pass
    # fallback: quick web-audio ping (cashier-ish triad)
    st.components.v1.html(
        """
        <script>
          (function(){
            try {
              ...
              // simple two-note 'cha-ching' impression
              tone(880, 0.08);
              setTimeout(()=>tone(1318.5, 0.12), 120);
            } catch(e){}
          })();
        </script>
        """,
...
def render_bucket_ui(current: int, total: int):
    ...
    st.components.v1.html(
        ...
        <div class='bucket-wrap'>
          ...
          <div class='bucket-label'>Gold in bucket: {current}</div>
        </div>
        """,
        height=70,
    )
...
if submitted:
    ...
    if correct:
        ...
        bucket_add_bar(N, st.session_state.current_points)

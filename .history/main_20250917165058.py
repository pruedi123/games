...
def tts_bytes(word: str, lang: str = "en", slow: bool = False) -> bytes | None:
    ...
    try:
        mp3 = io.BytesIO()
        gTTS(text=pron_text(word), lang=lang, slow=slow).write_to_fp(mp3)
        return mp3.getvalue()
    except Exception:
        return None
...
def say_slow_sentence(word: str, delay_ms: int = 0, rate: float = 0.55):
    ...
    speak_sentence = sentence_display.replace(word, pron_text(word))
    st.components.v1.html(
        f"""
        <script>
          (function() {{
...
def render_listen_and_spell():
    ...
    if rc1.button("🔊 Say it again", use_container_width=True):
        # Play once now, then speak a slow sentence after ~0.9s
        say_word_now(target)
    ...
    if rc2.button("🔁 3× Slow", use_container_width=True):
        # Repeat 3× very slowly with ~1s gaps, then speak a slow sentence after the repeats
        say_word_repeat(target, times=3, exaggerated=True, slow_rate=0.32, gap_ms=1000)
        total_delay = 3 * (900 + 1000)  # per_play_ms (~0.9s) + gap (1.0s), times 3

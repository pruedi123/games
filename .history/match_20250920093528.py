...
size_mode = st.sidebar.radio("Card size mode", ["Auto", "Exact 2 in"], index=1,
                             help="Exact 2 in makes each card ~192×192 px (CSS inches).")
...
# If user wants an exact physical size, force cards to ~2 inches (CSS 96 px/in)
EXACT_MODE = (size_mode == "Exact 2 in")
if EXACT_MODE:
    CARD_PX = int(2 * 96)  # 2 inches in CSS px
    HEIGHT_PX = CARD_PX
    # base font for button (fallback)
    FONT_PX = int(CARD_PX * 0.55)
    # big emoji size to fill the square nicely
    EMOJI_PX = int(CARD_PX * 0.72)
    PAD_PX = 0
    GAP_PX = 8
...
if EXACT_MODE:
    st.markdown(
        f"""
        <style>
        /* Exact 2 inch square cards */
        #cardgrid .stButton > button {{
            width: {CARD_PX}px !important;
            height: {CARD_PX}px !important;
            min-width: {CARD_PX}px !important;
            min-height: {CARD_PX}px !important;
            padding: 0 !important;
            margin: 6px auto !important; /* center within column */
            display: flex !important;
            align-items: center !important;
            justify-content: center !important;
            overflow: visible !important;
            font-size: {FONT_PX}px !important; /* fallback if emoji selector misses */
            line-height: 1 !important;
        }}
        #cardgrid .stButton > button *,
        #cardgrid .stButton > button p,
        #cardgrid .stButton > button span,
        #cardgrid .stButton > button div {{
    ...
    st.markdown(
        f"""
        <style>
        /* Auto mode: square, responsive cards */
        #cardgrid .stButton > button {{
...
# ----------------------------- LAYOUT -----------------------------
# Column count — in Exact mode we can safely use more columns since width is fixed
if EXACT_MODE:
    cols = 4 if pairs >= 6 else 3
else:
    # Fewer columns for bigger cards (ensures height grows, not just width)
    if size_choice in ("Giant",):
        cols = 1
    elif size_choice in ("Mega", "Huge", "XL"):
        cols = 2
    elif size_choice == "Large":
        cols = 3
    else:
...
# ----------------------------- RENDER GRID -----------------------------
st.markdown("<div id='cardgrid'>", unsafe_allow_html=True)
idx = 0
for _ in range(rows):
    ...
    for c in row:
        ...
        c.button(
            ...
            use_container_width=not EXACT_MODE,

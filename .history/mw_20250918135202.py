...
# If there is pending feedback/visuals from the last submission, play them now
pf = st.session_state.get("pending_feedback")
pc = st.session_state.get("pending_confetti", False)
if pf:
    if pf == 'right':
...
if submitted:
    ...
    correct = (g == target)
    if correct:
        st.session_state.pending_feedback = 'right'
        ...
        st.session_state.pending_feedback = 'wrong'
        st.session_state.pending_confetti = False
    ...
    # Always advance to the next word (form submit triggers one rerun automatically)
